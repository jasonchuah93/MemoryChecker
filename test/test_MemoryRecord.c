...
void test_allocateMemoryTest_should_allocate_user_input_size_and_return_pointer_which_point_to_first_allocated_memory_block(void){
    Allocation *ptr;
    MemoryBlock1 ptrBlock = {.header[49] = HEADERCONTENT , .memory[99] = "abcdef", .footer[49] = FOOTERCONTENT};
    _malloc_ExpectAndReturn((HEADER_SIZE+100+FOOTER_SIZE),(char*)ptrBlock.header);
	...
	TEST_ASSERT_EQUAL_STRING(HEADERCONTENT,ptr->headerAddr);
    TEST_ASSERT_EQUAL_STRING("abcdef",ptr->memoryAllocateAddr);
    TEST_ASSERT_EQUAL_STRING(FOOTERCONTENT,ptr->footerAddr);
...
void test_createRecord_should_create_descriptor_with_allocated_size_and_memory(void){
    ...
    TEST_ASSERT_EQUAL_STRING(HEADERCONTENT,ptrRecord->headerAddr);
    TEST_ASSERT_EQUAL_STRING("abcdef123456",ptrRecord->memoryAllocateAddr);
    TEST_ASSERT_EQUAL_STRING(FOOTERCONTENT,ptrRecord->footerAddr);
